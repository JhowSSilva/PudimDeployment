    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <p class="text-sm font-medium text-info-400">Credenciais AWS</p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-neutral-800 rounded-lg shadow p-8">
                ...
                <p class="text-neutral-400 mb-6">Escolha a conta AWS que será usada para provisionar o servidor</p>

                @if($credentials->isEmpty())
                    <div class="bg-warning-900/20 border border-yellow-200 rounded-lg p-6 text-center">
                        ...
                        <p class="text-neutral-400 mb-4">Você precisa adicionar uma conta AWS antes de provisionar servidores</p>
                        <a href="{{ route('aws-credentials.create') }}" class="inline-block bg-info-600 hover:bg-info-700 text-white px-6 py-2 rounded font-medium">
                ...
                @else
                    <form action="{{ route('aws-provision.step2') }}" method="POST">
                            ...
                            @foreach($credentials as $credential)
                                <label class="relative">
                                    <input 
                                        ...
                                        required
                                    >
                                    <div class="border-2 border-neutral-600 peer-checked:border-info-600 peer-checked:bg-info-900/20 rounded-lg p-4 cursor-pointer hover:border-neutral-400 transition">
                                        ...
                                        <p class="text-sm text-neutral-400 mb-2">{{ $credential->default_region }}</p>
                                        <div class="flex items-center text-sm text-neutral-500">
                                            <code class="text-xs bg-neutral-700 px-2 py-1 rounded">{{ $credential->masked_access_key }}</code>
                                        </div>
                                        @if($credential->description)
                                            <p class="text-sm text-neutral-400 mt-2">{{ $credential->description }}</p>
                                        @endif
                                    </div>
                                </label>
                            @endforeach
                        ...
                        <div class="flex justify-between items-center pt-6 border-t">
                            <a href="{{ route('servers.index') }}" class="text-neutral-400 hover:text-neutral-800">
                                ← Cancelar
                            </a>
