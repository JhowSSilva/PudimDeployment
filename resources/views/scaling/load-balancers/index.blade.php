<x-layout title="Load Balancers">
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('scaling.load-balancers.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
            Novo Load Balancer
        </a>
    </div>
                    ...
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-2">
                                ...
                                <span class="px-3 py-1 rounded text-xs font-semibold {{ $lb->status === 'active' ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300' }}">
                                    {{ ucfirst($lb->status) }}
                                </span>
                            </div>
                        ...
                        <div class="ml-4 flex gap-2">
                            <a href="{{ route('scaling.load-balancers.show', $lb) }}" class="px-3 py-1 bg-neutral-600 text-white rounded text-sm hover:bg-neutral-500">
                                Ver
                            </a>
                            <a href="{{ route('scaling.load-balancers.edit', $lb) }}" class="px-3 py-1 bg-blue-600 text-white rounded text-sm hover:bg-blue-700">
                                Editar
                            </a>
                        </div>
                    </div>
