        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('scaling.policies.store') }}" class="space-y-6">
                        @csrf
                        ...
                        <div class="border-b border-gray-200 pb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 shadow-sm focus:outline-none hover:border-blue-500">
                                    <input type="radio" name="type" value="{{ $value }}" {{ old('type', 'cpu') == $value ? 'checked' : '' }} 
                        ...
                        <div id="metric_fields" class="border-b border-gray-200 pb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div id="metric_field">
                                    ...
                                    <input type="text" name="metric" id="metric" value="{{ old('metric', 'cpu_percent') }}"
                                        placeholder="e.g., cpu_percent, memory_percent"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('metric')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div id="threshold_up_field">
                                    <label for="threshold_up" class="block text-sm font-medium text-gray-700">Scale Up Threshold (%) *</label>
                                    <input type="number" name="threshold_up" id="threshold_up" value="{{ old('threshold_up', 80) }}" min="0" max="100" step="0.1"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('threshold_up')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    <p class="mt-1 text-xs text-gray-500">Scale up when metric exceeds this value</p>
                                </div>

                                <div id="threshold_down_field">
                                    <label for="threshold_down" class="block text-sm font-medium text-gray-700">Scale Down Threshold (%) *</label>
                                    <input type="number" name="threshold_down" id="threshold_down" value="{{ old('threshold_down', 20) }}" min="0" max="100" step="0.1"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('threshold_down')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    <p class="mt-1 text-xs text-gray-500">Scale down when metric falls below this value</p>
                                </div>

                                <div>
                                    <label for="evaluation_periods" class="block text-sm font-medium text-gray-700">Evaluation Periods *</label>
                                    <input type="number" name="evaluation_periods" id="evaluation_periods" value="{{ old('evaluation_periods', 2) }}" min="1"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('evaluation_periods')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    <p class="mt-1 text-xs text-gray-500">Consecutive periods before triggering</p>
                                </div>

                                <div>
                                    <label for="period_duration" class="block text-sm font-medium text-gray-700">Period Duration (seconds) *</label>
                                    <input type="number" name="period_duration" id="period_duration" value="{{ old('period_duration', 60) }}" min="10"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('period_duration')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    <p class="mt-1 text-xs text-gray-500">Duration of each evaluation period</p>
                                </div>
                            </div>

                            <!-- Schedule Field (hidden by default) -->
                            <div id="schedule_field" class="hidden mt-6">
                                ...
                                <textarea name="schedule" id="schedule" rows="4"
                                    ...
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono text-sm">{{ old('schedule') }}</textarea>
                                @error('schedule')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                                <p class="mt-1 text-xs text-gray-500">Define scaling schedule in JSON format</p>
                            </div>
                        </div>
                        ...
                        <div class="border-b border-gray-200 pb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="number" name="scale_up_by" id="scale_up_by" value="{{ old('scale_up_by', 1) }}" min="1"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('scale_up_by')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    <p class="mt-1 text-xs text-gray-500">Number of servers to add</p>
                                </div>

                                <div>
                                    <label for="scale_down_by" class="block text-sm font-medium text-gray-700">Scale Down By *</label>
                                    <input type="number" name="scale_down_by" id="scale_down_by" value="{{ old('scale_down_by', 1) }}" min="1"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('scale_down_by')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    <p class="mt-1 text-xs text-gray-500">Number of servers to remove</p>
                                </div>

                                <div>
                                    <label for="min_servers" class="block text-sm font-medium text-gray-700">Minimum Servers *</label>
                                    <input type="number" name="min_servers" id="min_servers" value="{{ old('min_servers', 1) }}" min="1"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('min_servers')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="number" name="max_servers" id="max_servers" value="{{ old('max_servers', 10) }}" min="1"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('max_servers')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="number" name="cooldown_minutes" id="cooldown_minutes" value="{{ old('cooldown_minutes', 5) }}" min="1"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('cooldown_minutes')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    <p class="mt-1 text-xs text-gray-500">Wait time between scaling actions</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pb-6">
                            <label class="flex items-center">
                                ...
                                <span class="ml-2 text-sm text-gray-700">Activate policy immediately</span>
                            </label>
                        </div>

                        <!-- Form Actions -->
                        <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200">
                            <a href="{{ route('scaling.policies.index') }}" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
                                Cancel
                            </a>
    ...
    <script>
        function updateMetricFields(type) {
            ...
            if (type === 'schedule') {
                ...
                // Update metric name based on type
                const metricInput = document.getElementById('metric');
                if (type === 'cpu') {
                    metricInput.value = 'cpu_percent';
                } else if (type === 'memory') {
                    metricInput.value = 'memory_percent';
                }
            }
        }
    </script>
