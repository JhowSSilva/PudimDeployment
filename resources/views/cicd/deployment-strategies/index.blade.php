<x-layout title="Estratégias de Deployment">
    ...
    <!-- Info Box -->
    <div class="mb-6 p-4 bg-info-900/30 border border-blue-700 rounded-lg text-info-300">
        ...
        <div class="p-6 text-neutral-300">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap gap-2 mb-2">
                                <span class="px-3 py-1 rounded text-xs font-semibold
                                    {{ $strategy->type === 'blue_green' ? 'bg-info-900 text-info-300' : '' }}
                                    {{ $strategy->type === 'canary' ? 'bg-warning-900 text-warning-300' : '' }}
                                    {{ $strategy->type === 'rolling' ? 'bg-success-900 text-success-300' : '' }}
                                    {{ $strategy->type === 'recreate' ? 'bg-neutral-600 text-neutral-300' : '' }}">
                                    {{ ucfirst(str_replace('_', ' ', $strategy->type)) }}
                                </span>
                                @if($strategy->is_default)
                                    <span class="px-2 py-1 rounded text-xs bg-primary-900 text-primary-300">Padrão</span>
                                @endif
                            </div>
                        ...
                        <div class="ml-4 flex gap-2">
                            @if(!$strategy->is_default)
                                <form action="{{ route('cicd.deployment-strategies.make-default', $strategy) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="px-3 py-1 bg-primary-600 text-white rounded text-sm hover:bg-primary-700">
                                        Tornar Padrão
                                    </button>
                                </form>
                            @endif
                            <a href="{{ route('cicd.deployment-strategies.show', $strategy) }}" class="px-3 py-1 bg-neutral-600 text-white rounded text-sm hover:bg-neutral-7000">
                                Detalhes
                            </a>
                        </div>
                    </div>
            ...
            @empty
                <div class="text-center py-8 text-neutral-500">
                    ...
                    <a href="{{ route('cicd.deployment-strategies.create') }}" class="text-info-400 hover:text-info-300 mt-2 inline-block">
