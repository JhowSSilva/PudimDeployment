    ...
    <div class="bg-neutral-800 overflow-hidden shadow-xl sm:rounded-lg">
        <form action="{{ route('cicd.pipelines.store') }}" method="POST" class="p-6" x-data="{ triggerType: 'push' }">
            ...
            <div class="mb-4" x-show="triggerType === 'schedule'">
                ...
                <input type="text" name="trigger_config[cron]" value="{{ old('trigger_config.cron') }}" placeholder="0 0 * * *" class="w-full px-3 py-2 bg-neutral-700 border border-neutral-600 text-white rounded-md focus:border-primary-500 focus:ring focus:ring-primary-500 focus:ring-opacity-50">
                <p class="text-neutral-500 text-xs mt-1">Exemplo: 0 0 * * * (diariamente à meia-noite)</p>
            </div>

            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="auto_deploy" value="1" {{ old('auto_deploy') ? 'checked' : '' }} class="mr-2 bg-neutral-700 border-neutral-600 text-info-400 focus:ring-primary-500">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="timeout" value="{{ old('timeout', 3600) }}" min="60" class="w-full px-3 py-2 bg-neutral-700 border border-neutral-600 text-white rounded-md focus:border-primary-500 focus:ring focus:ring-primary-500 focus:ring-opacity-50">
            </div>

            <div class="flex justify-end">
                <button type="submit" class="px-4 py-2 bg-info-600 text-white rounded-md hover:bg-info-700 focus:outline-none focus:ring-2 focus:ring-primary-500">
                    Criar Pipeline
                </button>
            </div>
        </form>
