        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('scaling.load-balancers.store') }}" class="space-y-6">
                        @csrf
                        ...
                        <div class="border-b border-gray-200 pb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="port" id="port" value="{{ old('port', 80) }}" min="1" max="65535"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('port')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 shadow-sm focus:outline-none hover:border-blue-500">
                                        <input type="radio" name="algorithm" value="round_robin" {{ old('algorithm', 'round_robin') == 'round_robin' ? 'checked' : '' }} class="sr-only">
                                        <span class="flex flex-1">
                                            <span class="flex flex-col">
                                                <span class="block text-sm font-medium text-gray-900">Round Robin</span>
                                                <span class="mt-1 flex items-center text-sm text-gray-500">Distributes requests equally across all servers</span>
                                            </span>
                                        </span>
                                    </label>

                                    <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 shadow-sm focus:outline-none hover:border-blue-500">
                                        <input type="radio" name="algorithm" value="least_connections" {{ old('algorithm') == 'least_connections' ? 'checked' : '' }} class="sr-only">
                                        <span class="flex flex-1">
                                            <span class="flex flex-col">
                                                <span class="block text-sm font-medium text-gray-900">Least Connections</span>
                                                <span class="mt-1 flex items-center text-sm text-gray-500">Routes to server with fewest active connections</span>
                                            </span>
                                        </span>
                                    </label>

                                    <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 shadow-sm focus:outline-none hover:border-blue-500">
                                        <input type="radio" name="algorithm" value="ip_hash" {{ old('algorithm') == 'ip_hash' ? 'checked' : '' }} class="sr-only">
                                        <span class="flex flex-1">
                                            <span class="flex flex-col">
                                                <span class="block text-sm font-medium text-gray-900">IP Hash</span>
                                                <span class="mt-1 flex items-center text-sm text-gray-500">Routes same IP to same server consistently</span>
                                            </span>
                                        </span>
                                    </label>

                                    <label class="relative flex cursor-pointer rounded-lg border border-gray-300 bg-white p-4 shadow-sm focus:outline-none hover:border-blue-500">
                                        <input type="radio" name="algorithm" value="weighted" {{ old('algorithm') == 'weighted' ? 'checked' : '' }} class="sr-only">
                                        <span class="flex flex-1">
                                            <span class="flex flex-col">
                                                <span class="block text-sm font-medium text-gray-900">Weighted</span>
                                                <span class="mt-1 flex items-center text-sm text-gray-500">Distributes based on server weights</span>
                                            </span>
                                        </span>
                                    </label>
                                </div>
                        ...
                        <div class="border-b border-gray-200 pb-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div id="ssl_fields" class="{{ old('ssl_enabled') ? '' : 'hidden' }} space-y-4 pl-6">
                                    <div>
                                        ...
                                        <textarea name="ssl_certificate" id="ssl_certificate" rows="4"
                                            ...
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono text-xs">{{ old('ssl_certificate') }}</textarea>
                                    ...
                                    <div>
                                        ...
                                        <textarea name="ssl_private_key" id="ssl_private_key" rows="4"
                                            ...
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono text-xs">{{ old('ssl_private_key') }}</textarea>
                        ...
                        <div class="border-b border-gray-200 pb-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div id="health_check_fields" class="{{ old('health_check_enabled', true) ? '' : 'hidden' }} space-y-4">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            <label for="health_check_path" class="block text-sm font-medium text-gray-700">Health Check Path</label>
                                            <input type="text" name="health_check_path" id="health_check_path" value="{{ old('health_check_path', '/health') }}"
                                                placeholder="/health"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="health_check_interval" id="health_check_interval" value="{{ old('health_check_interval', 30) }}" min="5"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="health_check_timeout" id="health_check_timeout" value="{{ old('health_check_timeout', 5) }}" min="1"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="healthy_threshold" id="healthy_threshold" value="{{ old('healthy_threshold', 2) }}" min="1"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                            @error('healthy_threshold')
                                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                            @enderror
                                            <p class="mt-1 text-xs text-gray-500">Consecutive successes before marking healthy</p>
                                        </div>

                                        <div>
                                            <label for="unhealthy_threshold" class="block text-sm font-medium text-gray-700">Unhealthy Threshold</label>
                                            <input type="number" name="unhealthy_threshold" id="unhealthy_threshold" value="{{ old('unhealthy_threshold', 3) }}" min="1"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div class="pb-6">
                            ...
                            <div class="space-y-4">
                                <label class="flex items-center">
                                    <input type="checkbox" name="sticky_sessions" value="1" {{ old('sticky_sessions') ? 'checked' : '' }}
                                ...
                                <div id="session_ttl_field" class="{{ old('sticky_sessions') ? '' : 'hidden' }} pl-6">
                                    <label for="session_ttl" class="block text-sm font-medium text-gray-700">Session TTL (seconds)</label>
                                    <input type="number" name="session_ttl" id="session_ttl" value="{{ old('session_ttl', 3600) }}" min="60"
                                        class="mt-1 block w-full md:w-1/2 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('session_ttl')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                    <p class="mt-1 text-xs text-gray-500">How long to keep sessions bound to the same server</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200">
                            <a href="{{ route('scaling.load-balancers.index') }}" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
                                Cancel
                            </a>
