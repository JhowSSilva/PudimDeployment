<x-layout title="Editar Estratégia de Deployment">
    ...
    <div class="bg-neutral-800 overflow-hidden shadow-xl sm:rounded-lg">
        <form action="{{ route('cicd.deployment-strategies.update', $deploymentStrategy) }}" method="POST" class="p-6" x-data="{ strategyType: '{{ $deploymentStrategy->type }}' }">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="type" x-model="strategyType" disabled class="w-full px-3 py-2 bg-neutral-600 border border-neutral-500 text-neutral-400 rounded-md cursor-not-allowed">
                ...
                <p class="text-neutral-500 text-xs mt-1">O tipo não pode ser alterado após a criação.</p>
            </div>
            ...
            <div class="mb-4 p-4 bg-neutral-700 rounded" x-show="strategyType === 'blue_green'">
                ...
                <div>
                    ...
                    <input type="number" name="config[blue_green][wait_time]" value="{{ old('config.blue_green.wait_time', $deploymentStrategy->config['blue_green']['wait_time'] ?? 30) }}" class="w-full px-3 py-2 bg-neutral-600 border border-neutral-500 text-white rounded-md">
                </div>
            </div>
            ...
            <div class="mb-4 p-4 bg-neutral-700 rounded" x-show="strategyType === 'canary'">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="config[canary][initial_percentage]" value="{{ old('config.canary.initial_percentage', $deploymentStrategy->config['canary']['initial_percentage'] ?? 10) }}" min="1" max="100" class="w-full px-3 py-2 bg-neutral-600 border border-neutral-500 text-white rounded-md">
                </div>
                <div class="mb-3">
                    <label class="block text-neutral-300 text-sm mb-2">Incremento (%)</label>
                    <input type="number" name="config[canary][increment]" value="{{ old('config.canary.increment', $deploymentStrategy->config['canary']['increment'] ?? 25) }}" min="1" max="100" class="w-full px-3 py-2 bg-neutral-600 border border-neutral-500 text-white rounded-md">
                </div>
                <div>
                    <label class="block text-neutral-300 text-sm mb-2">Intervalo entre incrementos (minutos)</label>
                    <input type="number" name="config[canary][interval_minutes]" value="{{ old('config.canary.interval_minutes', $deploymentStrategy->config['canary']['interval_minutes'] ?? 5) }}" class="w-full px-3 py-2 bg-neutral-600 border border-neutral-500 text-white rounded-md">
                </div>
            </div>
            ...
            <div class="mb-4 p-4 bg-neutral-700 rounded" x-show="strategyType === 'rolling'">
                ...
                <div>
                    ...
                    <input type="number" name="config[rolling][max_batch_size]" value="{{ old('config.rolling.max_batch_size', $deploymentStrategy->config['rolling']['max_batch_size'] ?? 1) }}" min="1" class="w-full px-3 py-2 bg-neutral-600 border border-neutral-500 text-white rounded-md">
                </div>
            </div>
            ...
            <div class="mb-4 p-4 bg-neutral-700 rounded">
                ...
                <div class="mb-3">
                    ...
                    <input type="url" name="health_check[url]" value="{{ old('health_check.url', $deploymentStrategy->health_check['url'] ?? '') }}" class="w-full px-3 py-2 bg-neutral-600 border border-neutral-500 text-white rounded-md">
                </div>
                <div class="mb-3">
                    <label class="block text-neutral-300 text-sm mb-2">Tentativas</label>
                    <input type="number" name="health_check[retries]" value="{{ old('health_check.retries', $deploymentStrategy->health_check['retries'] ?? 3) }}" min="1" class="w-full px-3 py-2 bg-neutral-600 border border-neutral-500 text-white rounded-md">
                </div>
                <div>
                    <label class="block text-neutral-300 text-sm mb-2">Timeout (segundos)</label>
                    <input type="number" name="health_check[timeout]" value="{{ old('health_check.timeout', $deploymentStrategy->health_check['timeout'] ?? 30) }}" min="1" class="w-full px-3 py-2 bg-neutral-600 border border-neutral-500 text-white rounded-md">
                </div>
            </div>
        ...
        <form id="delete-form" action="{{ route('cicd.deployment-strategies.destroy', $deploymentStrategy) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
