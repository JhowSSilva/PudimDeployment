<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            @if($role->is_system)
                <div class="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-yellow-800">Função do Sistema</h3>
                            <p class="mt-1 text-sm text-yellow-700">
                                Esta é uma função do sistema e não pode ser modificada. Esta página é apenas para visualização.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-blue-700">
                                <strong>{{ $role->users_count }}</strong> {{ $role->users_count === 1 ? 'usuário atribuído' : 'usuários atribuídos' }} a esta função.
                                As alterações afetarão suas permissões imediatamente.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <form method="POST" action="{{ route('team.roles.update', $role) }}" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <div class="mb-4">
                            ...
                            <input 
                                ...
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 {{ $role->is_system ? 'bg-gray-100 cursor-not-allowed' : '' }}"
                            >
                            @error('name')
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-3">
                                <input 
                                    ...
                                    name="color" 
                                    id="color" 
                                    value="{{ old('color', $role->color ?? '#3b82f6') }}"
                                    {{ $role->is_system ? 'disabled' : '' }}
                                    class="h-10 w-20 rounded border border-gray-300 {{ $role->is_system ? 'cursor-not-allowed' : 'cursor-pointer' }}"
                                >
                                ...
                                <div class="border border-gray-200 rounded-lg p-4">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        @foreach($categoryPermissions as $permission)
                                            <label class="flex items-start p-3 hover:bg-gray-50 rounded-md {{ $role->is_system ? 'cursor-not-allowed' : 'cursor-pointer' }} transition">
                                                <input 
                                                    type="checkbox" 
                                                    name="permissions[]" 
                                                    value="{{ $permission->slug }}"
                                                    {{ in_array($permission->slug, old('permissions', $role->permissions ?? [])) ? 'checked' : '' }}
                                                    {{ $role->is_system ? 'disabled' : '' }}
