    ...
    <div class="bg-neutral-800 overflow-hidden shadow-xl sm:rounded-lg">
        <form action="{{ route('cicd.pipelines.update', $pipeline) }}" method="POST" class="p-6" x-data="{ triggerType: '{{ $pipeline->trigger_type }}' }">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" value="{{ old('name', $pipeline->name) }}" required class="w-full px-3 py-2 bg-neutral-700 border border-neutral-600 text-white rounded-md focus:border-primary-500 focus:ring focus:ring-primary-500 focus:ring-opacity-50">
                @error('name')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="3" class="w-full px-3 py-2 bg-neutral-700 border border-neutral-600 text-white rounded-md focus:border-primary-500 focus:ring focus:ring-primary-500 focus:ring-opacity-50">{{ old('description', $pipeline->description) }}</textarea>
            </div>

            <div class="mb-4">
                <label class="block text-neutral-300 text-sm font-bold mb-2">Tipo de Trigger</label>
                <select name="trigger_type" x-model="triggerType" required class="w-full px-3 py-2 bg-neutral-700 border border-neutral-600 text-white rounded-md focus:border-primary-500 focus:ring focus:ring-primary-500 focus:ring-opacity-50">
            ...
            <div class="mb-4" x-show="triggerType === 'schedule'">
                ...
                <input type="text" name="trigger_config[cron]" value="{{ old('trigger_config.cron', $pipeline->trigger_config['cron'] ?? '') }}" placeholder="0 0 * * *" class="w-full px-3 py-2 bg-neutral-700 border border-neutral-600 text-white rounded-md focus:border-primary-500 focus:ring focus:ring-primary-500 focus:ring-opacity-50">
            </div>

            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="auto_deploy" value="1" {{ old('auto_deploy', $pipeline->auto_deploy) ? 'checked' : '' }} class="mr-2 bg-neutral-700 border-neutral-600 text-info-400 focus:ring-primary-500">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="timeout" value="{{ old('timeout', $pipeline->timeout) }}" min="60" class="w-full px-3 py-2 bg-neutral-700 border border-neutral-600 text-white rounded-md focus:border-primary-500 focus:ring focus:ring-primary-500 focus:ring-opacity-50">
            </div>
