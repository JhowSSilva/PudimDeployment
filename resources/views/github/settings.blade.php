    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-2xl mx-auto">
            ...
            @if(auth()->user()->hasGitHubConnected())
                <!-- GitHub Conectado -->
                <div class="bg-success-900/40 border border-green-700 rounded-lg p-6 mb-6">
                    <div class="flex items-start justify-between">
                        <div class="flex items-center">
                            <svg class="w-12 h-12 text-success-400 mr-4" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"/>
                            </svg>
                            <div>
                                <h3 class="text-lg font-semibold text-green-100">GitHub Conectado</h3>
                                <p class="text-success-300">
                                    @<strong>{{ auth()->user()->github_username }}</strong>
                                </p>
                                <p class="text-sm text-success-300 mt-1">Conexão ativa e funcionando</p>
                            </div>
                        </div>
                        <form action="{{ route('github.disconnect') }}" method="POST" onsubmit="return confirm('Tem certeza que deseja desconectar seu GitHub?')">
                            @csrf
                            <button type="submit" class="px-4 py-2 bg-error-500 text-white rounded hover:bg-error-700 transition">
                                Desconectar
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="mt-4 pt-4 border-t border-green-700">
                        <a href="{{ route('github.repositories.index') }}" class="inline-flex items-center text-success-300 hover:text-green-100 font-medium">
                ...
                <div class="bg-neutral-800 border border-neutral-700 rounded-lg shadow-lg p-6 mb-6">
                    ...
                    <div class="border border-neutral-700 rounded-lg p-5 mb-4">
                        <div class="flex items-center mb-3">
                            <div class="bg-info-900/40 rounded-full p-2 mr-3">
                            ...
                            <div>
                                ...
                                <span class="text-xs bg-info-900/40 text-info-300 px-2 py-1 rounded">Recomendado</span>
                            </div>
                        </div>
                        
                        <div class="bg-info-900/20 border border-blue-700 rounded p-4 mb-4 text-sm">
                            <h4 class="font-semibold text-info-300 mb-2">📋 Como gerar seu token:</h4>
                            <ol class="list-decimal list-inside space-y-1 text-info-300">
                                ...
                                <li>Selecione os scopes:
                                    <ul class="list-disc list-inside ml-6 mt-1">
                                        <li><code class="bg-info-900/40 px-1 rounded">repo</code> (controle total de repositórios)</li>
                                        <li><code class="bg-info-900/40 px-1 rounded">workflow</code> (gerenciar GitHub Actions)</li>
                                        <li><code class="bg-info-900/40 px-1 rounded">admin:repo_hook</code> (gerenciar webhooks)</li>
                                    </ul>
                                </li>
                        ...
                        <form action="{{ route('github.personal-token') }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <input 
                                    ...
                                    name="token" 
                                    id="token"
                                    class="w-full border border-neutral-600 bg-neutral-900 text-white rounded px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('token') border-red-500 @enderror font-mono text-sm"
                            ...
                            <button type="submit" class="w-full bg-info-600 text-white px-4 py-3 rounded font-medium hover:bg-info-700 transition">
                    ...
                    @if(config('services.github.client_id'))
                        ...
                        <div class="border border-neutral-700 rounded-lg p-5">
                            ...
                            <a href="{{ route('github.connect') }}" class="block w-full bg-neutral-800 text-white px-4 py-3 rounded font-medium hover:bg-neutral-900 transition text-center">
