<div class="bg-neutral-800 rounded-lg shadow p-6">
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold text-neutral-100">Cost Forecast</h3>
    ...
    @if(isset($forecast['error']))
        <div class="bg-error-900/20 border border-red-200 text-error-400 px-4 py-3 rounded">
            {{ $forecast['error'] }}
        </div>
    @else
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4">
                ...
                <div class="text-xs text-info-400 mt-1">
            ...
            <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-4">
                ...
                <div class="text-xs text-purple-700 mt-1">
                    ...
                    @endphp
                    @if($diff > 0)
                        <span class="text-error-400">↑ +{{ number_format($percentChange, 1) }}%</span>
                    @elseif($diff < 0)
                        <span class="text-success-400">↓ {{ number_format($percentChange, 1) }}%</span>
                    @else
        ...
        @if(isset($forecast['breakdown']) && count($forecast['breakdown']) > 0)
            <div class="border-t border-neutral-700 pt-4">
                <h4 class="text-sm font-semibold text-neutral-300 mb-3">Cost Breakdown by Server</h4>
                <div class="space-y-2">
                    @foreach($forecast['breakdown'] as $serverCost)
                        <div class="flex items-center justify-between p-3 bg-neutral-900 rounded-lg">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    <div class="text-sm font-medium text-neutral-100">{{ $serverCost['server_name'] }}</div>
        ...
        @if(isset($forecast['trend']))
            <div class="mt-4 p-4 bg-info-900/20 rounded-lg">
                <div class="flex items-start space-x-3">
                    <svg class="w-5 h-5 text-info-400 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
