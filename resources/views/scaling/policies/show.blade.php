<x-layout title="Política: {{ $policy->name }}">
    <div class="mb-6 flex justify-between items-center">
        ...
        <div class="flex gap-2">
            <a href="{{ route('scaling.policies.edit', $policy) }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                Editar
            </a>
            <a href="{{ route('scaling.policies.index') }}" class="px-4 py-2 bg-neutral-600 text-white rounded-lg hover:bg-neutral-500">
                Voltar
            </a>
        </div>
    </div>
        ...
        <div class="bg-neutral-800 p-6 rounded-lg">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="px-3 py-1 rounded text-xs font-semibold {{ $policy->is_active ? 'bg-green-900 text-green-300' : 'bg-neutral-600 text-neutral-300' }}">
            ...
            @if($policy->serverPool)
                <div class="p-4 bg-neutral-700 rounded-lg">
                    ...
                    <div class="space-y-1 text-sm text-neutral-400">
                        <div>
                            <span>Status:</span>
                            <span class="ml-1 px-2 py-0.5 rounded text-xs {{ $policy->serverPool->status === 'active' ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300' }}">
                    ...
                    <a href="{{ route('scaling.pools.show', $policy->serverPool) }}" class="mt-3 inline-block text-blue-400 hover:text-blue-300 text-sm">
                        Ver Pool →
                    </a>
                </div>
            @else
        ...
        <div class="space-y-2">
            @forelse($policy->executions()->latest()->limit(10)->get() as $execution)
                <div class="p-3 bg-neutral-700 rounded flex justify-between items-center">
                    ...
                    <span class="px-3 py-1 rounded text-xs font-semibold {{ $execution->status === 'success' ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300' }}">
