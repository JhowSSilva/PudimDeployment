...
<div x-data="{ show: false }"
     ...
     class="fixed inset-0 z-50 flex items-center justify-center bg-neutral-950/70 backdrop-blur-sm"
     role="status"
     aria-live="polite"
     style="display: none;">
    
    <div class="bg-neutral-800 border border-neutral-700 rounded-lg shadow-2xl p-8 max-w-sm w-full mx-4">
        <div class="flex flex-col items-center gap-4">
            ...
            @if($attributes->get('cancellable'))
                <button @click="show = false; $dispatch('loading-cancelled')"
                        class="mt-2 px-4 py-2 text-sm text-neutral-400 hover:text-neutral-200 transition-colors duration-200">
                    Cancelar
                </button>
            @endif
        </div>
