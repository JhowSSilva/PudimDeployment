            ...
            <div class="flex justify-between items-start mb-4">
                <div>
                    <span class="px-3 py-1 rounded text-sm
                        {{ $pipelineRun->status === 'success' ? 'bg-success-900 text-success-300' : '' }}
                        {{ $pipelineRun->status === 'failed' ? 'bg-error-900 text-error-300' : '' }}
                        {{ $pipelineRun->status === 'running' ? 'bg-info-900 text-info-300 animate-pulse' : '' }}
                        {{ $pipelineRun->status === 'pending' ? 'bg-neutral-600 text-neutral-300' : '' }}
                        {{ $pipelineRun->status === 'canceled' ? 'bg-warning-900 text-warning-300' : '' }}">
                ...
                <div class="flex gap-2">
                    @if($pipelineRun->status === 'running')
                        <form action="{{ route('cicd.pipeline-runs.cancel', $pipelineRun) }}" method="POST">
                            @csrf
                            <button type="submit" class="px-4 py-2 bg-warning-500 text-white rounded hover:bg-warning-700">
                                Cancelar Execução
                            </button>
                        </form>
                    @elseif($pipelineRun->status === 'failed')
                        <form action="{{ route('cicd.pipeline-runs.retry', $pipelineRun) }}" method="POST">
                            @csrf
                            <button type="submit" class="px-4 py-2 bg-info-600 text-white rounded hover:bg-info-700">
                                Tentar Novamente
                            </button>
                        </form>
                    @endif
                ...
                <div class="mb-4 p-4 bg-neutral-700 rounded-lg border border-neutral-600">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="px-2 py-1 rounded text-xs
                            {{ $stageResult['status'] === 'success' ? 'bg-success-900 text-success-300' : '' }}
                            {{ $stageResult['status'] === 'failed' ? 'bg-error-900 text-error-300' : '' }}
                            {{ $stageResult['status'] === 'running' ? 'bg-info-900 text-info-300' : '' }}
                    ...
                    @if(isset($stageResult['output']))
                        <details class="mt-2">
                            <summary class="cursor-pointer text-info-400 hover:text-info-300 text-sm">Ver logs</summary>
                    ...
                    @if(isset($stageResult['error']))
                        <div class="mt-2 p-3 bg-error-900/20 border border-red-700 rounded">
                            <div class="text-error-300 text-sm font-semibold mb-1">Erro:</div>
                            <pre class="text-error-300 text-xs overflow-x-auto">{{ $stageResult['error'] }}</pre>
                        </div>
                    @endif
                </div>
            @endforeach
