<x-layout title="Aprovação #{{ $deploymentApproval->id }}">
    ...
    @if(session('error'))
        <div class="mb-4 p-4 bg-error-900/50 border border-red-500 text-error-300 rounded-lg">
            {{ session('error') }}
        </div>
    @endif
            ...
            <div class="flex justify-between items-start mb-4">
                <div>
                    <span class="px-4 py-2 rounded text-sm font-semibold
                        {{ $deploymentApproval->status === 'pending' ? 'bg-warning-900 text-warning-300' : '' }}
                        {{ $deploymentApproval->status === 'approved' ? 'bg-success-900 text-success-300' : '' }}
                        {{ $deploymentApproval->status === 'rejected' ? 'bg-error-900 text-error-300' : '' }}
                ...
                @if($deploymentApproval->status === 'pending' && $deploymentApproval->canUserApprove(auth()->user()))
                    <div class="flex gap-2">
                        <form action="{{ route('cicd.deployment-approvals.approve', $deploymentApproval) }}" method="POST">
                            @csrf
                            <button type="submit" class="px-4 py-2 bg-success-500 text-white rounded hover:bg-success-700">
                                Aprovar
                            </button>
                        </form>
                        <form action="{{ route('cicd.deployment-approvals.reject', $deploymentApproval) }}" method="POST">
                            @csrf
                            <button type="submit" class="px-4 py-2 bg-error-500 text-white rounded hover:bg-error-700">
                                Rejeitar
                            </button>
                        </form>
                    </div>
        ...
        <div class="p-6 text-neutral-300">
            ...
            <div class="mb-4">
                ...
                <div class="w-full bg-neutral-700 rounded-full h-2">
                    <div class="bg-success-500 h-2 rounded-full" style="width: {{ (count($deploymentApproval->approval_history ?? []) / $deploymentApproval->required_approvers) * 100 }}%"></div>
                </div>
            </div>

            @if($deploymentApproval->hasEnoughApprovals())
                <div class="p-3 bg-success-900/30 border border-green-700 rounded text-success-300 text-sm">
                    ✓ Aprovações suficientes obtidas! O deployment pode prosseguir.
                </div>
            @elseif($deploymentApproval->status === 'pending')
                <div class="p-3 bg-warning-900/30 border border-yellow-700 rounded text-warning-300 text-sm">
                            ...
                            <div class="flex justify-between items-start">
                                ...
                                <div class="text-right">
                                    <span class="px-2 py-1 rounded text-xs font-semibold
                                        {{ $history['action'] === 'approved' ? 'bg-success-900 text-success-300' : 'bg-error-900 text-error-300' }}">
            ...
            <div class="space-y-2">
                <div>
                    ...
                    <span class="ml-2 px-2 py-1 rounded text-xs
                        {{ $deploymentApproval->pipelineRun->status === 'success' ? 'bg-success-900 text-success-300' : '' }}
                        {{ $deploymentApproval->pipelineRun->status === 'failed' ? 'bg-error-900 text-error-300' : '' }}
                        {{ $deploymentApproval->pipelineRun->status === 'running' ? 'bg-info-900 text-info-300' : '' }}
                ...
                @endif
                <div>
                    <a href="{{ route('cicd.pipeline-runs.show', $deploymentApproval->pipelineRun) }}" class="text-info-400 hover:text-info-300 text-sm">
