<x-layout>
    <div class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            ...
            <h1 class="mt-2 text-3xl font-bold text-neutral-100">Create New Server</h1>
            <p class="mt-2 text-sm text-neutral-300">Configure your server with modern multi-language support</p>
        </div>
        ...
        <div class="mb-8">
            <nav aria-label="Progress">
                <ol role="list" class="space-y-4 md:flex md:space-y-0 md:space-x-8">
                    ...
                    <li class="md:flex-1">
                        <div class="group pl-4 py-2 flex flex-col border-l-4 border-neutral-700 md:pl-0 md:pt-4 md:pb-0 md:border-l-0 md:border-t-4">
        ...
        <form id="server-form" action="{{ route('servers.store') }}" method="POST" class="bg-neutral-800 shadow sm:rounded-lg">
            @csrf
            
            <!-- Step 1: Basic Configuration -->
            <div id="step-1" class="step px-4 py-5 sm:p-6 space-y-6">
                <h3 class="text-lg font-medium text-neutral-100">Server Details</h3>
                
                <!-- Name -->
                <div>
                    <label for="name" class="block text-sm font-medium text-neutral-300">Server Name</label>
                ...
                <!-- IP Address -->
                <div>
                    <label for="ip_address" class="block text-sm font-medium text-neutral-300">IP Address</label>
                    <input type="text" name="ip_address" id="ip_address" value="{{ old('ip_address') }}" required 
                        placeholder="192.168.1.100"
                        class="mt-1 block w-full rounded-md border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm @error('ip_address') border-error-600 @enderror">
                    @error('ip_address')
                        <p class="mt-1 text-sm text-error-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-2 gap-6">
                    <!-- SSH Port -->
                    <div>
                        <label for="ssh_port" class="block text-sm font-medium text-neutral-300">SSH Port</label>
                        <input type="number" name="ssh_port" id="ssh_port" value="{{ old('ssh_port', 22) }}" required
                            class="mt-1 block w-full rounded-md border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm @error('ssh_port') border-error-600 @enderror">
                        @error('ssh_port')
                            <p class="mt-1 text-sm text-error-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- OS -->
                    <div>
                        <label for="os" class="block text-sm font-medium text-neutral-300">Operating System</label>
                        <select name="os" id="os" required
                            class="mt-1 block w-full rounded-md border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm @error('os') border-error-600 @enderror">
                ...
                <div>
                    <label for="type" class="block text-sm font-medium text-neutral-300">Server Type</label>
                    <select name="type" id="type" required
                        class="mt-1 block w-full rounded-md border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm @error('type') border-error-600 @enderror">
            ...
            <div id="step-2" class="step hidden px-4 py-5 sm:p-6 space-y-6">
                <h3 class="text-lg font-medium text-neutral-100">Programming Language</h3>
                
                <!-- Language Grid -->
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    @foreach($languages as $lang => $langData)
                        <div class="language-option border-2 border-neutral-700 rounded-lg p-4 cursor-pointer hover:border-primary-500 transition-colors"
                             data-language="{{ $lang }}"
                             onclick="selectLanguage('{{ $lang }}')">
                            <div class="flex flex-col items-center text-center">
                                <div class="w-16 h-16 mb-3 flex items-center justify-center bg-neutral-700 rounded-full">
                                    @if($lang === 'php')
                                    ...
                                    @else
                                        <div class="w-8 h-8 bg-neutral-9000 rounded"></div>
                                    @endif
                                </div>
                                <h4 class="font-medium text-neutral-100">{{ $langData['name'] }}</h4>
                ...
                <div id="version-selection" class="hidden space-y-4">
                    <div class="border-t pt-4">
                        <h4 class="font-medium text-neutral-100 mb-4">Select Version</h4>
                        <select name="language_version" id="language_version" 
                            class="mt-1 block w-full rounded-md border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
            ...
            <div id="step-3" class="step hidden px-4 py-5 sm:p-6 space-y-6">
                <h3 class="text-lg font-medium text-neutral-100">Stack Configuration</h3>
            ...
            <!-- Navigation Buttons -->
            <div class="px-4 py-3 bg-neutral-900 text-right sm:px-6 flex justify-between">
                <button type="button" id="prev-btn" 
                    class="hidden inline-flex justify-center py-2 px-4 border border-neutral-600 shadow-sm text-sm font-medium rounded-md text-neutral-300 bg-neutral-700 hover:bg-neutral-700  focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                    Previous
                </button>
                ...
                <div class="flex space-x-3">
                    ...
                    <button type="submit" id="submit-btn" 
                        class="hidden inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-success-500 hover:bg-success-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                        Create Server
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function selectLanguage(language) {
            ...
            document.querySelectorAll('.language-option').forEach(el => {
                ...
                el.classList.add('border-neutral-700', 'dark:border-neutral-600');
            });
            
            const selectedOption = document.querySelector(`[data-language="${language}"]`);
            selectedOption.classList.remove('border-neutral-700', 'dark:border-neutral-600');
        ...
        function loadStackConfiguration(language) {
            ...
            let html = `
                <!-- Web Server -->
                <div>
                    <label class="block text-sm font-medium text-neutral-300">Web Server</label>
                    <select name="webserver" class="mt-1 block w-full rounded-md border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                ...
                <!-- Database -->
                <div>
                    <label class="block text-sm font-medium text-neutral-300">Database</label>
                    <select name="database_type" class="mt-1 block w-full rounded-md border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                        <option value="">None</option>
                ...
                <!-- Cache -->
                <div>
                    <label class="block text-sm font-medium text-neutral-300">Cache Service</label>
                    <select name="cache_service" class="mt-1 block w-full rounded-md border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                        <option value="">None</option>
            ...
            if (language === 'php') {
                html += `
                    <div class="border-t pt-4">
                        <h4 class="font-medium text-neutral-100 mb-4">PHP Configuration</h4>
                        <label class="flex items-center">
                            <input type="checkbox" name="install_composer" value="1" checked class="rounded border-neutral-600 text-primary-600 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                            <span class="ml-2 text-sm text-neutral-300">Install Composer</span>
                        </label>
                    </div>
                `;
                ...
                html += `
                    <div class="border-t pt-4">
                        <h4 class="font-medium text-neutral-100 mb-4">Node.js Configuration</h4>
                        <div class="space-y-2">
                            <label class="flex items-center">
                                <input type="checkbox" name="install_yarn" value="1" checked class="rounded border-neutral-600 text-primary-600 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                                <span class="ml-2 text-sm text-neutral-300">Install Yarn</span>
                            </label>
                            <label class="flex items-center">
                                <input type="checkbox" name="install_pm2" value="1" checked class="rounded border-neutral-600 text-primary-600 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                                <span class="ml-2 text-sm text-neutral-300">Install PM2</span>
                            </label>
                        </div>
                    </div>
                `;
                ...
                html += `
                    <div class="border-t pt-4">
                        <h4 class="font-medium text-neutral-100 mb-4">Python Configuration</h4>
                        <div class="space-y-2">
                            <label class="flex items-center">
                                <input type="checkbox" name="install_poetry" value="1" class="rounded border-neutral-600 text-primary-600 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                                <span class="ml-2 text-sm text-neutral-300">Install Poetry</span>
                            </label>
                            <label class="flex items-center">
                                <input type="checkbox" name="install_pipenv" value="1" class="rounded border-neutral-600 text-primary-600 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                                <span class="ml-2 text-sm text-neutral-300">Install Pipenv</span>
                            </label>
                        </div>
                    </div>
                `;
        ...
        document.getElementById('next-btn').addEventListener('click', () => {
            if (currentStep === 1) {
                ...
                requiredFields.forEach(field => {
                    ...
                    if (!input.value.trim()) {
                        valid = false;
                        input.classList.add('border-error-600');
                    } else {
                        input.classList.remove('border-error-600');
                    }
                });
