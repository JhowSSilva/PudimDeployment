                    ...
                    <div class="flex items-start space-x-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-xs text-red-700">{{ \Carbon\Carbon::parse($threat->detected_at)->diffForHumans() }}</span>
                            </div>
                            <p class="text-sm text-red-700 mt-1">{{ $threat->description }}</p>
                            @if($threat->severity)
                                <span class="inline-block mt-2 px-2 py-1 text-xs font-semibold rounded 
                                    {{ $threat->severity === 'critical' ? 'bg-red-700 text-white' : '' }}
                                    {{ $threat->severity === 'high' ? 'bg-orange-600 text-white' : '' }}
                                    {{ $threat->severity === 'medium' ? 'bg-yellow-600 text-white' : '' }}
                                    {{ $threat->severity === 'low' ? 'bg-blue-600 text-white' : '' }}">
                ...
                @foreach($blockedIps as $ip)
                    <div class="flex items-center justify-between p-3 bg-neutral-50 border border-neutral-200 rounded-lg">
                        ...
                        <button 
                            ...
                            class="px-3 py-1 text-xs font-medium text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded">
                            Unblock
                        </button>
                    </div>
                @endforeach
