                                ...
                                <div class="flex items-center space-x-2">
                                    @can('update', $team)
                                        ...
                                        <form action="{{ route('teams.members.remove', [$team, $member]) }}" method="POST" onsubmit="return confirm('Remover este membro?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="px-3 py-1 bg-red-500 hover:bg-red-600 text-white text-sm rounded-lg transition-all">
                                                Remover
                                            </button>
                                        </form>
                                    @else
                                    ...
                                    <div class="bg-white rounded-lg p-4 border border-neutral-200">
                                        <div class="flex items-start justify-between gap-4">
                                            <div class="flex-1 min-w-0">
                                                <div class="font-semibold text-neutral-900">{{ $invite->email }}</div>
                                                <div class="text-sm text-neutral-600 mt-1">
                                                    Convidado por {{ $invite->inviter->name }} · 
                                                    {!! $invite->role_badge !!} ·
                                                    Expira em {{ $invite->expires_at->diffForHumans() }}
                                                </div>
                                                <div class="text-xs text-neutral-500 mt-2 font-mono bg-neutral-50 p-2 rounded border border-neutral-100 break-all">
                                                    {{ $invite->invite_url }}
                                                </div>
                                            </div>
